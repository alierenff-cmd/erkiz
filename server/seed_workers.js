...
async function seed() {
    ...
    const tempZip = path.join(__dirname, 'temp_seed.zip');
    const tmpDir = path.join(__dirname, 'scratch_seed');
    ...
    rows.forEach((r, idx) => {
        ...
        if (rawDate.includes('.') || rawDate.includes('/') || rawDate.includes('-')) {
            ...
            bdateStr = excelSerialToDate(rawDate);
        }
    ...
    console.log(`Extracted ${workersToInsert.length} valid worker records from Excel.`);
    ...
    console.log(`SUCCESS! ${inserted} worker records successfully seeded into MySQL database!`);
    await pool.end();
}
